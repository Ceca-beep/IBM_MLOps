pipeline {
    ...
    steps {
        sh '''
            sudo podman rm -f prometheus || true
            sudo podman run -d --name prometheus \
                --network llm-net \
                -p 9090:9090 \
                localhost/my-prometheus:latest
        '''
    }
}
...
stage('Start Grafana') {
    steps {
        sh '''
            sudo podman rm -f grafana || true
            sudo podman run -d --name grafana \
                --network llm-net \
                -p 4000:3000 \
